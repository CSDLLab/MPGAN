...
def add_noise(MAX_BOX_WIDTH, mark, forground):
    ...
    for i in range(noise_num):
        ...
        for j in range(len(z_list)):
            ...
            # print(value)
            if mark[pos_z + z_temp][pos_x + x_temp][pos_y + y_temp] != 1:
                noise_image[pos_z + z_temp][pos_x + x_temp][pos_y + y_temp] = value
...
def getRandChildNumber():
    ''' Random generate children number of a tree node
        ...
        Output:
            (int) : Children number
    '''
    return np.random.choice([1, 2, 3, 4], p=[0.35, 0.5, 0.1, 0.05])
    ...
    while len(dq):
        ...
        if root_depth < MAX_TREE_DEPTH:
            ...
            if root_node.idx == 1:  # 根节点单独处理
                ...
                if root_node.idx == 2:
                    child_num = np.random.choice([2, 3, 4], p=[0.35, 0.5, 0.15])
                else:
                    child_num = getRandChildNumber()
                child_angles_range = Draw3DTools.sliceRange(0, 360, child_num)
